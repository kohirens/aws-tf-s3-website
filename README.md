...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_lambda_origin"></a> [lambda\_origin](#module\_lambda\_origin) | git@github.com:kohirens/aws-tf-lambda-function//. | 2.1.0 |
...
| <a name="input_cf_additional_origins"></a> [cf\_additional\_origins](#input\_cf\_additional\_origins) | Additional origins to add to the distribution, please note that the key will be used for the origin\_id. | <pre>map(object({ # these properties are a mix of ordered_cache_behavior and origin arguments.<br>    domain_name              = string<br>    cache_policy_id          = string<br>    origin_request_policy_id = optional(string)<br>    path_pattern             = string<br>    query_string             = bool<br>    custom_origin_config = optional(object({<br>      http_port                = string<br>      https_port               = string<br>      origin_protocol_policy   = string<br>      origin_ssl_protocols     = list(string)<br>      origin_keepalive_timeout = optional(number)<br>      origin_read_timeout      = optional(number)<br>    }))<br>    s3_origin_config = optional(object({<br>      origin_access_identity = string<br>    }))<br>  }))</pre> | `{}` | no |
