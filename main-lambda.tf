...
module "lambda_origin" {
  source = "github.com/kohirens/aws-tf-lambda-function//.?ref=2.1.2"
...
resource "aws_lambda_permission" "allow_cloudfront" {
  ...
  statement_id  = "AllowCloudFrontServicePrincipalExecute"
...
resource "aws_lambda_permission" "allow_cloudfront2" {
  depends_on             = [module.lambda_origin]
  statement_id           = "AllowCloudFrontServicePrincipalInvoke"
